...
const DEFAULT_CONNECTIONS: { from: [number, number]; to: [number, number] }[] =
  [
    ...
    { from: [28.61, 77.21], to: [25.2, 55.27] },
...
export function InteractiveGlobe({
  ...
  const draw = useCallback(() => {
    ...
    glowGrad.addColorStop(0, "rgba(34, 212, 245, 0.03)");
    glowGrad.addColorStop(1, "rgba(34, 212, 245, 0)");
    ctx.fillStyle = glowGrad;
    ctx.fillRect(0, 0, w, h);

    ctx.beginPath();
    ctx.arc(cx, cy, radius, 0, Math.PI * 2);
    ctx.strokeStyle = "rgba(34, 212, 245, 0.06)";
    ctx.lineWidth = 1;
    ctx.stroke();
