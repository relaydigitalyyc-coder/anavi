...
const stablecoinBackings = [
  ...
  },
  {
    ...
    backingAssets: [
      ...
      {
        asset: "Residential Portfolio",
        percentage: 30,
        value: "$13.5M",
        location: "Miami Beach",
      },
      {
        asset: "Development Rights",
        percentage: 10,
        value: "$4.5M",
        location: "Brooklyn",
      },
  ...
  const totalPortfolioValue =
    dbCrypto.length > 0
      ...
      : capabilities.allowDemoFixtures
        ? 11216675
        : 0;
  const activeStablecoinBackings = capabilities.allowDemoFixtures
    ? stablecoinBackings
    : [];
  const activeTokenizationPipeline = capabilities.allowDemoFixtures
    ...
    <div className="p-4 md:p-6 space-y-6">
      ...
      >
        <TabsList className="grid w-full grid-cols-3 md:w-auto md:inline-grid">
          <TabsTrigger value="stablecoins" className="text-xs md:text-sm">
            Stablecoins
          </TabsTrigger>
          <TabsTrigger value="portfolio" className="text-xs md:text-sm">
            Portfolio
          </TabsTrigger>
          <TabsTrigger value="tokenization" className="text-xs md:text-sm">
            Tokenization
          </TabsTrigger>
        </TabsList>
        ...
        <TabsContent value="stablecoins" className="space-y-4">
          {activeStablecoinBackings.map(coin => (
              ...
              <CardHeader className="pb-2">
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-2">
                  ...
                  <div className="flex items-center gap-2">
                    <Badge
                      variant={
                        coin.status === "Fully Collateralized" ||
                        coin.status === "Over-Collateralized"
                          ? "default"
                          : "secondary"
                      }
        ...
        <TabsContent value="portfolio" className="space-y-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Active cryptocurrency positions and performance
              </CardDescription>
            </CardHeader>
            <CardContent>
                ...
                <Table className="w-full">
                  <TableHeader>
                    <TableRow className="border-b text-left text-sm text-muted-foreground">
                      ...
                      <TableHead className="pb-3 font-medium hidden md:table-cell">
                        Holdings
                      </TableHead>
                      <TableHead className="pb-3 font-medium hidden md:table-cell">
                        Avg Cost
                    ...
                    {cryptoPositions.map(position => (
                      ...
                      >
                        ...
                        <TableCell className="py-4 text-right">
                          <div
                            className={`flex items-center justify-end gap-1 ${position.isPositive ? "text-green-500" : "text-red-500"}`}
                          >
                            {position.isPositive ? (
                              <ArrowUpRight className="h-4 w-4" />
                            ) : (
        ...
        <TabsContent value="tokenization" className="space-y-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Real-world assets being tokenized for fractional ownership
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              {activeTokenizationPipeline.map(asset => (
