...
export async function notifyNewMatch(
  ...
  const msg = matchReason
    ? `Match: ${matchReason.slice(0, 80)}${matchReason.length > 80 ? "…" : ""}`
    : "A new compatible match was found for your intent.";
...
export async function createVerificationDocument(
  data: typeof verificationDocuments.$inferInsert
) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
...
export async function getDashboardStats(userId: number) {
  ...
  const [
    ...
    db
      .select({
        previousScore: trustScoreHistory.previousScore,
        newScore: trustScoreHistory.newScore,
      })
  ...
  const latestHistory = recentTrustHistory[0];
  const trustScoreDelta = latestHistory
    ? Number(latestHistory.newScore) - Number(latestHistory.previousScore ?? 0)
    : 0;
